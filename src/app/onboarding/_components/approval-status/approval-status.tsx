  ...
  const steps = [
    {
      ...
      description:
        "Successfully submitted your details, we will review and get back to you within 24 hours",
    },
    {
      isDone: true,
      title: "Review Application",
      description:
        "Your application has passed the standard phase and has been moved to another level",
    },
    {
      isDone: false,
  ...
  return (
    <>
      <div className="max-w-preview-width mx-auto">
        <h1 className="font-inter text-custom-primary md:text-xl text-lg p-[10px]">
          PENDING APPROVAL
        </h1>
        <p className=" mb-[63px] font-plus-jakarta-sans text-white text-base">
          Thank you for signing up on AirPlay, we will get back to you shortly
          as we are reviewing your application
          <br />{" "}
          <span className="text-white">
            {" "}
            You will receive an email about your application int the next 24
            hours.
          </span>
        </p>
          ...
          {steps.map((step) => (
            <li key={step.title} className="flex flex-col gap-5">
              <div className="flex items-center gap-2">
                ...
                <h2
                  className={cn(
                    "font-plus-jakarta-sans font-medium text-base text-white",
                    !step.isDone && "text-white/70"
                  )}
                >
                  {step.title}
                </h2>
              </div>
              <p className="text-custom-footer_border font-inter font-normal text-xs">
                {step.description}
              </p>
            </li>
          ))}
