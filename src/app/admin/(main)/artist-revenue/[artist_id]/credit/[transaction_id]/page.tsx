		...
		<div className="container mx-auto p-4 md:p-6 lg:p-8 space-y-8">
			...
			<div className="p-6 rounded-lg bg-card border shadow-sm space-y-6">
				...
				<div className="space-y-4">
					...
					<Input placeholder="Enter description" value={notes} onChange={e => setNotes(e.target.value)} className="h-10 text-base pl-7 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" min="0" step="0.01" disabled={isUpdatingCredit} />

					{/* Update Button */}
					<div className="flex justify-center">
						<Button className="bg-primary hover:bg-primary/90" onClick={handleUpdate} disabled={isUpdatingCredit || !finalAmount || parseFloat(finalAmount) < 0}>
							{isUpdatingCredit ? 'Updating...' : 'Add to Account'}
							{isUpdatingCredit ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <ChevronRight className="h-4 w-4 " />}
						</Button>
					</div>
				</div>
			</div>
