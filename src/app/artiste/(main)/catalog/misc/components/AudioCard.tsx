...
import { Play, Pause, Ellipsis, Eye, Trash, Edit2, ArrowRight, ImagePlus } from 'lucide-react';
...
const AudioCard = ({ audio, album, selected }: { audio: TArtistMedia; album?: TArtisteAlbum; selected?: boolean }) => {
	...
	const { state: isConfirmAddToAlbumDialogOpen, setTrue: openConfirmAddToAlbumDialog, setState: setIsConfirmAddToAlbumDialogState, setFalse: closeConfirmAddToAlbumDialog } = useBooleanStateControl();
	...
	return (
		<article
			className="relative flex flex-col gap-2.5 rounded-xl cursor-pointer p-1 border-[0.6px]"
			onClick={() => {
				if (!!album) {
		...
		>
			{selected && (
				<aside className="flex flex-col items-center justify-center z-[10] absolute size-full top-0 left-0 bottom-0 right-0 bg-black/60">
					<TickCircle size={40} variant="Bold" fill="white" style={{ fill: 'white' }} />
					<small className="font-medium text-sm text-white">Already in album</small>
				</aside>
			)}
							...
							<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
								<div className="flex flex-col items-center gap-3 mb-2">
									...
									<div className="relative w-32 h-32 rounded-lg overflow-hidden border-2 border-dashed border-border hover:border-primary transition-colors cursor-pointer group" onClick={() => coverArtInputRef.current?.click()}>
										<Image src={coverArtPreview || audio.mediaCoverArtUrl || '/images/placeholder.png'} alt="Cover art" className="object-cover" fill />
			...
			<CustomAlertDialog variant={'default'} open={isConfirmAddToAlbumDialogOpen} onOpenChange={setIsConfirmAddToAlbumDialogState} title={'Confirm Add To album'} description={'Are you sure you want to add this song to album' + ' ' + album?.title + '?'} actionLabel={'Add to Album'} cancelLabel="Cancel" onAction={handleConfirmAddToAblum} onCancel={closeConfirmAddToAlbumDialog} showCancel={true} showAction={true} isPerformingAction={isAddingToAlbum} />
			<CustomAlertDialog
				variant={'warning'}
