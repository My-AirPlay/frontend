		...
		<div className="relative flex flex-col h-full overflow-hidden">
			...
			<section className="flex-1 overflow-y-auto">
				{!isLoading && data?.data.length === 0 ? (
					...
					<div className="grid [grid-template-columns:repeat(auto-fill,minmax(200px,1fr))] gap-4 p-4">{isLoading ? Array.from({ length: 10 }).map((_, index) => <AlbumCardSkeleton key={`${_}${index}`} />) : data?.data.map(album => <AlbumCard key={album._id} album={album} />)}</div>
				)}
			</section>
