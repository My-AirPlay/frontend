...
import { useGetVideos } from '../api/getArtisteMedias';
			...
			<section className="flex-1 overflow-y-auto">
				{!isLoading && data?.data.length === 0 && (
					<div className="flex items-center justify-center h-[90%] w-full">
						<h1 className="text-xl text-muted-foreground">No videos found</h1>
					</div>
				)}
				<div className="grid [grid-template-columns:repeat(auto-fill,minmax(200px,1fr))] gap-4 p-4">{isLoading ? Array.from({ length: 10 }).map((_, index) => <VideoCardSkeleton key={`${_}${index}`} />) : data?.data.map(audio => <VideoCard key={audio._id} audio={audio} />)}</div>
			</section>
