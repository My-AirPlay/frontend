...
const AlbumCard = ({ album }: { album: TArtisteAlbum }) => {
	const { state: isViewDetailsDialogOpen, setTrue: openViewDetailsDialog, setState: setIsViewDetailsDialogState } = useBooleanStateControl();
	...
	const { state: isAddSongToAlbumDialogOpen, setTrue: openAddSongToAlbumModal, setState: setIsAddSongToAlbumDialogState } = useBooleanStateControl();

	const { state: isConfirmDeleteModalOpen, setTrue: openConfirmDeleteModal, setFalse: closeConfirmDeleteModal, setState: setConfirmDeleteModalState } = useBooleanStateControl();
	const handleDelete = () => {
	...
	return (
		<article className="relative flex flex-col gap-2.5 rounded-xl cursor-pointer p-1 border-[0.6px]">
			...
			<div className="flex items-center justify-center relative max-h-[150px] aspect-square">
				<Image src={album.mediaDirCoverArtUrl} alt={album.title} className="object-cover rounded-xl z-[2] text-opacity-0 text-[0px]" priority={true} fill onError={() => setImageError(true)} />
						...
						<div className="rounded-2xl bg-background p-2.5">
							<div className="flex gap-6 mb-4 p-4">
								<div className="relative w-32 h-32 flex items-center justify-center">
									<Image src={album.mediaDirCoverArtUrl} alt={album.title} className="object-cover rounded-lg text-opacity-0 text-[0px]" fill onError={() => setImageError(true)} />
									{imageError && <Musicnote size={60} className="stroke-white z-[3]" />}
								</div>

								<div className="flex flex-col justify-between">
									<div>
										<div className="text-orange-500 text-sm">Released • {album.releaseDate ? format(new Date(album.releaseDate), 'MMM dd yyyy') : 'No date'}</div>
						...
						<Form {...form}>
							<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormControl>
													<SelectSimple options={formattedData?.Genre} isLoadingOptions={isLoadingStaticData} value={field.value} valueKey="value" labelKey="label" onChange={field.onChange} placeholder="Select genre" hasError={!!errors.mainGenre} errormessage={errors.mainGenre?.message} />
												</FormControl>
											</FormItem>
										)}
									/>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormControl>
													<Textarea placeholder="Enter track description" className={`min-h-24 ${errors.description ? 'border-red-500' : ''}`} {...field} />
												</FormControl>
								...
								<div className="flex justify-center mt-8">
									<Button type="submit" size="lg" className="rounded-full" disabled={!isValid}>
			...
			<CustomAlertDialog variant={'warning'} open={isConfirmDeleteModalOpen} onOpenChange={setConfirmDeleteModalState} title={'Confirm Delete'} description={'Are you sure you want to delete this album track? This action cannot be undone.'} actionLabel={'Go ahead, delete'} cancelLabel="Cancel" onAction={handleDelete} onCancel={closeConfirmDeleteModal} showCancel={true} showAction={true} isPerformingAction={isDeletingAlbum} />
			<AddSongToAlbumFromAlbumMenu album={album} isAddSongToAlbumDialogOpen={isAddSongToAlbumDialogOpen} setIsAddSongToAlbumDialogState={setIsAddSongToAlbumDialogState} />
		</article>
	);
};
