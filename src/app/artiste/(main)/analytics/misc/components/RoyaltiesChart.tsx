		...
		<div className="bg-custom-gradient rounded-xl p-6 w-full max-md:max-h-[350px]">
			<div className="flex justify-between items-center mb-4">
				...
				<div className="flex items-center space-x-2">
					...
					<button className="bg-primary hover:bg-primary text-white px-3 py-1 rounded text-xs">View Report</button>
				</div>
			</div>
			...
			<div className="w-full h-full">
				<ChartContainer config={chartConfig}>
					<BarChart accessibilityLayer data={chartData}>
						...
						<Bar dataKey="spotify" stackId="a" fill="var(--color-spotify)" radius={[0, 0, 4, 4]} width={2} barSize={3.5} />
						<Bar dataKey="appple_music" stackId="a" fill="var(--color-appple_music)" radius={[4, 4, 0, 0]} width={2} barSize={3.5} />
						<Bar dataKey="youtube" stackId="a" fill="var(--color-youtube)" radius={[4, 4, 0, 0]} width={2} barSize={3.5} />
					</BarChart>
				</ChartContainer>
			</div>
		</div>
