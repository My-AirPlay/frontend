...
export default function ContactPage() {
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		window.location.href = `mailto:info@myairplay.com?subject=${encodedSubject}&body=${encodedBody}`;
		...
		<div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50 p-4">
			<div className="w-full max-w-xl bg-black rounded-lg p-6 relative">
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					<Input type="text" placeholder="Your Name" className="w-full  " value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} required />

					<Input type="email" placeholder="Your Email" className="w-full  " value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} required />

					<Textarea placeholder="Your Message" className="w-full   min-h-[120px]" value={formData.message} onChange={e => setFormData({ ...formData, message: e.target.value })} required />
