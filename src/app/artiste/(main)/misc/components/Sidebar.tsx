...
import { MusicSquare, Headphone, Category, Musicnote, Chart, Setting2 } from 'iconsax-react';
				...
				{sidebarItems.map((item, index) => {
					...
					return (
						<Link href={item.href} className={cn('relative flex md:flex-col gap-x-4 text-sm font-normal items-center md:justify-center py-4 pl-2 pr-3 rounded-l-xl rounded-r-none hover:rounded-r-xl cursor-pointer transition-all duration-200', isActive && 'bg-background !rounded-r-none')} key={index}>
							...
							<span>
								<item.icon variant="Bold" size={18} className={cn('max-md:hidden fill-primary', isActive ? 'fill-primary' : 'fill-muted-foreground')} />
								<item.icon variant="Bold" size={20} className={cn('md:hidden fill-primary', isActive ? 'fill-primary' : 'fill-muted-foreground')} />
							</span>
