...
export default function Step0MediaUploadSteps() {
	const { currentStep, setCurrentStep, mediaType } = useMediaUploadStore();
	...
	const getStepLabel = (label: string) => {
		if (label !== 'Music Cover') {
			return label;
		}
		return mediaType === 'Video' ? 'Thumbnail Cover' : label;
	};
	...
	return (
		<div className="px-4 py-6">
			<div className="flex justify-center items-center gap-8 flex-wrap mt-8">
				{steps.map((step, i) => (
					<div key={step.name} className={cn('step-item cursor-pointer relative flex flex-col items-center', getStepStatus(step.value))} onClick={() => handleStepClick(step.value)}>
						<div className={cn('step w-10 h-10 flex items-center justify-center rounded-full border border-gray-300', getStepStatus(step.value) === 'active' ? 'bg-primary text-white' : '', getStepStatus(step.value) === 'complete' ? 'bg-green-500 text-white' : '')}>{getStepStatus(step.value) === 'complete' ? <Check className="h-5 w-5" /> : i + 1}</div>
						<p className="text-center mt-2 text-sm">{getStepLabel(step.label)}</p>
					</div>
				))}
			</div>
		</div>
