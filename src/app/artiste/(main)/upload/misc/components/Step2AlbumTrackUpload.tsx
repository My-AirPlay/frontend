...
function TrackEditSheet({
  ...
  isOpen: boolean;
  onClose: () => void;
  track: AlbumTrackInfo | null;
  albumInfo: any;
  onSave: (data: TrackFormValues) => void;
  genreOptions: any[];
  streamingPlatformOptions: { label: string; value: string; }[];
...
export default function TrackUpload() {
  ...
  return (
    <div className="w-[80vw] sm:w-[65vw] max-w-[600px] md:max-w-3xl mx-auto mt-16">
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[0.7rem] text-white/50">
                      {track.fileType} •{" "}
                      {track.fileId && fileDetails[track.fileId]
                        ? getFileSize(fileDetails[track.fileId].size) + " MB"
                        : "Stored in IndexedDB"}
                    </span>
                  </div>
      ...
      <TrackEditSheet
        isOpen={editingTrackIndex !== null}
        onClose={closeEditTrack}
        track={editingTrack}
        albumInfo={albumInfo}
        onSave={saveTrackChanges}
        genreOptions={formattedData?.Genre || []}
        streamingPlatformOptions={formattedData?.StreamingPlatform || []}
        isLoadingOptions={isLoadingStaticAppInfo}
      />
    </div>
  )
}