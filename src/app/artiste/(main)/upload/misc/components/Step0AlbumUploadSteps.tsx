			...
			<div className="flex justify-center items-center gap-4 md:gap-8 flex-wrap mt-8">
				{steps.map((step, i) => (
					<div key={step.name} className={cn('step-item cursor-pointer relative flex flex-col items-center', getStepStatus(step.value))} onClick={() => handleStepClick(step.value)}>
						<div className={cn('step size-6 md:size-10 flex items-center justify-center rounded-full border border-gray-300 text-xs md:text-sm', getStepStatus(step.value) === 'active' ? 'bg-primary text-white' : '', getStepStatus(step.value) === 'complete' ? 'bg-green-500 text-white' : '')}>{getStepStatus(step.value) === 'complete' ? <Check className="h-5 w-5" /> : i + 1}</div>
