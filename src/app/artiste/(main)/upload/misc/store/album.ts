      ...
      (set, get) => ({
        ...
        togglePlatform: (platform) => {
          ...
          const updatedPlatforms = exists
            ? get().streamingPlatforms.filter((p) => p !== platform)
            : [...get().streamingPlatforms, platform]
        ...
        hasOngoingUpload: () => {
          const state = get()
          return (
            state.albumTracks.length > 0 ||
            state.mediaFileIds.length > 0 ||
            state.coverArtId !== null ||
            Object.values(state.albumInfo).some((val) => (val !== "" && Array.isArray(val) ? val.length > 0 : true))
          )
        },
      }),
      {
