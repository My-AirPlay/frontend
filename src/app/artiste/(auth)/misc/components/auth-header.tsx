...
const AuthHeader = () => {
	...
	return (
		<header className="flex gap-7 sticky top-0 justify-between items-center mb-2 py-4">
			...
			<nav className={cn('  lg:relative fixed top-0 left-0 lg:w-auto lg:flex-row flex flex-col max-w-none w-svw lg:h-auto h-svh lg:bg-transparent bg-custom-mobile-nav z-50 flex-1 lg:translate-x-0 -translate-x-full transition-transform duration-200 ease-in-out lg:justify-center ', showMenu && 'translate-x-0')}>
				<div className="lg:hidden flex justify-between pl-4 pr-3 pt-7 mb-9">
					<Button className="text-white bg-transparent hover:bg-transparent" onClick={() => setShowMenu(false)}>
