		...
		<div className={cn('', containerClass)}>
			<Popover open={open} onOpenChange={setOpen}>
				<div className="flex flex-col gap-2">
					...
					<PopoverTrigger className={cn('w-full !overflow-hidden whitespace-nowrap', inputVariants({ variant, inputSize: size, className }))} tabIndex={0} type="button" role="combobox" aria-controls="combobox-options" aria-expanded={open} onClick={() => setOpen(!open)} ref={triggerRef} disabled={disabled}>
						{customDisplay ? customDisplay : <span className={cn('!overflow-hidden text-sm w-full font-normal text-left truncate', values.length > 0 && options && options?.length ? '' : '!text-[#A4A4A4]', placeHolderClass)}>{showSelectedValues && values.length > 0 && options && options?.length ? getSelectedLabels() : placeholder}</span>}

						<ChevronDown stroke={triggerColor || '#FE6902'} className={cn('opacity-80', open && 'rotate-180')} />
					</PopoverTrigger>
				</div>
				...
				<PopoverContent className={cn('p-0 min-h-[25vh] max-h-[450px] overflow-y-scroll', triggerRef?.current && `min`)} style={{ ...contentStyle, width, maxWidth: width }}>
						...
						<CommandGroup className="flex flex-col gap-3 px-3 max-w-full" id="combobox-options">
							...
							{!isLoadingOptions && options && options?.length > 0 ? (
								optionsToDisplay?.map((option, index) => (
									<button className={cn('text-xs w-full relative flex select-none items-center rounded-md px-3 py-2 outline-none aria-selected:bg-primary-light aria-selected:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50', 'py-2 my-1 hover:!bg-primary-light hover:!text-primary cursor-pointer rounded-lg hover:border-transparent', itemClass, 'max-w-full text-left items-start')} key={index} type="button" onClick={() => handleSelect(String(option[valueKey]))}>
										{checkType === 'stroke' ? <Check className={cn('mr-2 h-4 w-4', values.includes(String(option[valueKey])) ? 'opacity-100' : 'opacity-0')} /> : <Checkbox checked={values.includes(String(option[valueKey]))} onChange={() => handleSelect(String(option[valueKey]))} name="checked" value={String(option[valueKey])} className="!w-4 !outline-2 !border-1.5 !h-4 rounded-sm mr-3" />}
					...
					{customFooterActions && <div className="sticky bottom-0">{customFooterActions}</div>}
				</PopoverContent>
			</Popover>
