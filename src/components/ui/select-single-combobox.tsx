...
const SelectSingleCombo = <T extends object>({ value, onChange, options, hasError, errormessage, label, name, placeholder, className, containerClass, itemClass, fullWidth, placeHolderClass, isLoadingOptions, valueKey, labelKey, triggerColor, showSelectedValue = true, variant }: SelectProps<T> & VariantProps<typeof inputVariants>) => {
				...
				<div className="flex flex-col gap-2">
					...
					<PopoverTrigger asChild>
						<button
							...
							ref={triggerRef}
						>
							<span className={cn('!overflow-hidden text-sm w-full font-normal !text-left truncate', value && options && options?.length ? '' : '!text-[#A4A4A4]', placeHolderClass)}>{showSelectedValue && value && options && options?.length ? getOptionLabel(options.find(option => option[valueKey] === String(value)) || ({} as T)) : placeholder}</span>
							<ChevronDown className={cn('ml-2 h-5 w-5 shrink-0 opacity-70 transition-transform duration-300', open && 'rotate-180')} stroke={triggerColor || '#FE6902'} />
						</button>
					</PopoverTrigger>
				</div>
				...
				<PopoverContent className={cn('p-0', triggerRef?.current && `min`)} style={{ width, maxWidth: width }}>
					<Command>
						<div className="relative px-3">
							...
							<input className="focus:!ring-0 !ring-0 bg-transparent pl-6 p-3 !outline-none text-sm placeholder:text-[#86898ec7] border-b border-[#E6E6E6] w-full rounded-none" placeholder={placeholder || 'Search'} type="text" onChange={e => setSearchText(e.target.value)} />
						</div>
						<CommandGroup className="flex flex-col gap-3 px-3 max-w-full" id="combobox-options">
							...
							{!isLoadingOptions && options && options?.length > 0 ? (
								optionsToDisplay?.map((option, index) => (
									<button className={cn('text-xs w-full relative flex select-none items-center rounded-md px-3 py-2 outline-none aria-selected:bg-primary-light aria-selected:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50', 'py-2 my-1 hover:!bg-primary-light hover:!text-primary cursor-pointer rounded-lg hover:border-transparent', itemClass, 'max-w-full text-left items-start')} key={index} onClick={() => handleSelect(option[valueKey] as string)} type="button">
										<CheckIcon className={cn('mr-2 h-4 w-4', option[valueKey] === value ? 'opacity-100' : 'opacity-0')} />
