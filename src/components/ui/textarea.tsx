	...
	(
		{
			...
			minRows = 2,
			maxRows = 10,
			onInput, // Destructure onInput from props
			...props
		},
		ref
	) => {
		...
		const handleInput = React.useCallback(
			(e: React.FormEvent<HTMLTextAreaElement>) => {
				...
				onInput?.(e); // Use destructured onInput
			},
			// Updated dependency array
			[autoResize, resizeTextArea, onInput]
		);

		return (
			<div className={containerClassName}>
				{label && (
					<label className={cn('text-sm font-medium mb-2.5', labelVariants[variant || 'default'])} htmlFor={props.id || label}>
						{label}
					</label>
				)}
				<div className="relative">
					<textarea
						className={cn(inputVariants({ variant, inputSize, className }), autoResize && 'overflow-hidden')}
						ref={node => {
