...
export async function setArtistAccessToken({ access, refresh }: { access: string; refresh: string }) {
...
export async function setAdminAccessToken({ access, refresh }: { access: string; refresh: string }) {
	const cookieStore = await cookies();
	setAxiosDefaultToken(access);
...
export async function getAdminRefreshToken() {
	const cookieStore = await cookies();
	return cookieStore.get(`${ADMIN_TOKEN_PREFIX}_REFRESH`)?.value;
}
